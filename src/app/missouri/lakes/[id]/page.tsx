...
import { missouriLakes, getMissouriLakeById, getMissouriLakeForRamp } from "@/data/missouri-lakes";
...
export function generateStaticParams() {
  return missouriLakes.map((l) => ({ id: l.id }));
}

export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  const { id } = await params;
  const lake = getMissouriLakeById(id);
  ...
  return {
    ...
    openGraph: { title: `${lake.name} Boat Ramps`, url: `https://www.rampseeker.com/missouri/lakes/${lake.id}` },
    twitter: { card: "summary", title: `${lake.name} Boat Ramps | RampSeeker` },
    alternates: { canonical: `https://www.rampseeker.com/missouri/lakes/${lake.id}` },
  };
}

export default async function MissouriLakePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const lake = getMissouriLakeById(id);
  if (!lake) notFound();

  const lakeRamps = unified.filter((r) => r.state === "MO" && getMissouriLakeForRamp(r.latitude, r.longitude)?.id === lake.id);
  const otherLakes = missouriLakes.filter((l) => l.id !== lake.id).slice(0, 6);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Home", item: "https://www.rampseeker.com" }, { "@type": "ListItem", position: 2, name: "Missouri", item: "https://www.rampseeker.com/missouri" }, { "@type": "ListItem", position: 3, name: lake.name, item: `https://www.rampseeker.com/missouri/lakes/${lake.id}` }] }) }} />
      ...
      <p className="text-gray-500 mb-6">{lakeRamps.length} boat ramps &middot; {lake.counties.join(", ")}, Missouri</p>
      ...
      <div className="mb-12">
        <LakeRampList ramps={lakeRamps} stateName="Missouri" />
      </div>
      ...
      <h2 className="font-[Cabin] text-xl font-bold text-charcoal mb-4">Other Missouri Lakes</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {otherLakes.map((l) => (
          <Link key={l.id} href={`/missouri/lakes/${l.id}`} className="group bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all border-l-4 border-l-water">
