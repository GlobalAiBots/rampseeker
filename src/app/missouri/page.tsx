...
export default function MissouriPage() {
  ...
  const lakeCounts = useMemo(() => {
    ...
    for (const l of missouriLakes) {
      map[l.id] = moRamps.filter((r) => getMissouriLakeForRamp(r.latitude, r.longitude)?.id === l.id).length;
    }
    return map;
  }, [moRamps]);
  ...
  return (
    <div>
      <section className="py-16 md:py-24 text-center px-4 bg-cream" style={{ backgroundImage: "radial-gradient(circle at 20% 80%, rgba(30,96,145,0.06) 0%, transparent 50%)" }}>
        ...
        <p className="text-gray-500 mt-4 max-w-lg mx-auto">{moRamps.length}+ boat ramps across {missouriLakes.length} major lakes. GPS coordinates, amenities, directions.</p>
      </section>

      <section className="max-w-6xl mx-auto px-4 pt-12 pb-8">
        <h2 className="font-[Cabin] text-2xl font-bold text-charcoal mb-4">Missouri Lakes</h2>
      ...
      <section className="max-w-6xl mx-auto px-4 pt-8 pb-8">
        <h2 className="font-[Cabin] text-xl font-bold text-charcoal mb-4">All {moRamps.length} Missouri Boat Ramps</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {display.map((r) => {
            const lake = getMissouriLakeForRamp(r.latitude, r.longitude);
            return (
