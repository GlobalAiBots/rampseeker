...
import { idahoLakes, getIdahoLakeById, getIdahoLakeForRamp } from "@/data/idaho-lakes";
import { unified } from "@/data/all-ramps";
import LakeRampList from "@/components/LakeRampList";
import CletusAd from "@/components/CletusAd";
import type { Metadata } from "next";

export function generateStaticParams() { return idahoLakes.map((l) => ({ id: l.id })); }

export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  const { id } = await params;
  const lake = getIdahoLakeById(id);
  if (!lake) return { title: "Not Found" };
  return {
    title: `${lake.name} Boat Ramps — Launch Sites & Access Points | RampSeeker`,
    description: `Every boat ramp on ${lake.name}, Idaho. GPS coordinates, amenities, directions.`,
    openGraph: { title: `${lake.name} Boat Ramps`, url: `https://www.rampseeker.com/idaho/lakes/${lake.id}` },
    twitter: { card: "summary", title: `${lake.name} Boat Ramps | RampSeeker` },
    alternates: { canonical: `https://www.rampseeker.com/idaho/lakes/${lake.id}` },
  };
}

export default async function IdahoLakePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const lake = getIdahoLakeById(id);
  if (!lake) notFound();
  const lakeRamps = unified.filter((r) => r.state === "ID" && getIdahoLakeForRamp(r.latitude, r.longitude)?.id === lake.id);
  const otherLakes = idahoLakes.filter((l) => l.id !== lake.id).slice(0, 6);
  const faqs = [
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Home", item: "https://www.rampseeker.com" }, { "@type": "ListItem", position: 2, name: "Idaho", item: "https://www.rampseeker.com/idaho" }, { "@type": "ListItem", position: 3, name: lake.name, item: `https://www.rampseeker.com/idaho/lakes/${lake.id}` }] }) }} />
      <nav className="text-sm text-gray-400 mb-6 flex flex-wrap gap-2"><Link href="/" className="hover:text-water transition">Home</Link><span>/</span><Link href="/idaho" className="hover:text-water transition">Idaho</Link><span>/</span><span className="text-charcoal font-medium">{lake.name}</span></nav>
      <h1 className="font-[Cabin] text-3xl md:text-4xl font-bold text-charcoal mb-2">Every Boat Ramp on {lake.name}</h1>
      <p className="text-gray-500 mb-6">{lakeRamps.length} boat ramps &middot; {lake.counties.join(", ")}, Idaho</p>
      ...
      <div className="mb-12"><LakeRampList ramps={lakeRamps} stateName="Idaho" /></div>
      ...
      <CletusAd />
      <h2 className="font-[Cabin] text-xl font-bold text-charcoal mb-4">Other Idaho Waters</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">{otherLakes.map((l) => (<Link key={l.id} href={`/idaho/lakes/${l.id}`} className="group bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all border-l-4 border-l-water"><p className="font-bold text-charcoal group-hover:text-water transition">{l.name}</p><p className="text-gray-500 text-sm">{l.nearestTowns[0]}</p></Link>))}</div>
    </div>
  );
}
