...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: `Find every boat ramp on ${lake.name} in Oklahoma. ${lake.acres.toLocaleString()} acres, ${lake.shorelineMiles} miles of shoreline. GPS coordinates, directions, and local tips.`,
...
export default async function LakePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  if (id === "grand-lake") notFound(); // handled by /grand-lake
  const lake = getLakeById(id);
  if (!lake) notFound();
  ...
  const lakeSchema = {
    ...
    address: { "@type": "PostalAddress", addressRegion: "Oklahoma", addressCountry: "US" },
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(lakeSchema) }} />
      ...
      {/* Hero */}
      <h1 className="font-[Cabin] text-3xl md:text-4xl font-bold text-charcoal mb-2">{lake.name} Boat Ramps</h1>
      <p className="text-gray-500 mb-6">{lakeRamps.length} boat ramp{lakeRamps.length !== 1 ? "s" : ""} &middot; {lake.counties.join(", ")} Count{lake.counties.length > 1 ? "ies" : "y"}, Oklahoma</p>
        ...
        {lakeRamps.map((r) => (
          <Link key={r.id} href={`/ramps/${r.id}`} className="group flex items-center justify-between bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all border-l-4 border-l-water">
            <div>
              ...
              <p className="text-gray-500 text-sm">{r.city}, OK{r.featured ? " \u00b7 Detailed Guide" : ""}</p>
            </div>
            <span className="text-sm font-semibold text-sunset">View &rarr;</span>
          </Link>
        ))}
        {lakeRamps.length === 0 && <p className="text-gray-400">No ramps found for this lake yet. <a href="mailto:hello@rampseeker.com" className="text-water">Know one? Submit it.</a></p>}
      </div>
