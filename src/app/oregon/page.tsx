...
export default function OregonPage() {
  ...
  const cityMap = ((precomputedCities as unknown) as Record<string, [string, number][]>)["oregon"] || [];
  ...
  return (
    <div>
      ...
      <section className="max-w-4xl mx-auto px-4 pt-8">
        <p className="text-gray-600 leading-relaxed text-sm md:text-base">Oregon offers {orRamps.length.toLocaleString()}+ public boat ramps across its waterways. From the Columbia River, Crater Lake, and the Oregon coast, the state provides excellent access for boaters, anglers, and kayakers. Popular catches include salmon, steelhead, and bass. <Link href="/blog/how-to-launch-a-boat-safely" className="text-water hover:underline">Learn how to launch safely</Link>.</p>
      </section>

      {/* State Map */}
      {(() => {
        const mapPins = orRamps.map(r => ({ id: r.id, name: r.name, latitude: r.latitude, longitude: r.longitude, city: r.city }));
        const center: [number, number] = orRamps.length > 0 ? [orRamps.reduce((s, r) => s + r.latitude, 0) / orRamps.length, orRamps.reduce((s, r) => s + r.longitude, 0) / orRamps.length] : [39.8, -98.5];
