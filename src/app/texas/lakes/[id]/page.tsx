...
import { texasLakes, getTexasLakeById, getTexasLakeForRamp } from "@/data/texas-lakes";
import { unified } from "@/data/all-ramps";
import CletusAd from "@/components/CletusAd";
import type { Metadata } from "next";

export function generateStaticParams() {
  return texasLakes.map((l) => ({ id: l.id }));
}

export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  const { id } = await params;
  const lake = getTexasLakeById(id);
  if (!lake) return { title: "Lake Not Found" };
  return {
    title: `${lake.name} Boat Ramps — Launch Sites & Access Points | RampSeeker`,
    description: `Every boat ramp on ${lake.name}, Texas. ${lake.acres.toLocaleString()} acres. GPS coordinates, amenities, directions.`,
    openGraph: { title: `${lake.name} Boat Ramps`, url: `https://rampseeker.com/texas/lakes/${lake.id}` },
    twitter: { card: "summary", title: `${lake.name} Boat Ramps | RampSeeker` },
    alternates: { canonical: `https://rampseeker.com/texas/lakes/${lake.id}` },
  };
}

export default async function TexasLakePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const lake = getTexasLakeById(id);
  if (!lake) notFound();

  const lakeRamps = unified.filter((r) => r.state === "TX" && getTexasLakeForRamp(r.latitude, r.longitude)?.id === lake.id);
  const otherLakes = texasLakes.filter((l) => l.id !== lake.id).slice(0, 6);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Home", item: "https://rampseeker.com" }, { "@type": "ListItem", position: 2, name: "Texas", item: "https://rampseeker.com/texas" }, { "@type": "ListItem", position: 3, name: lake.name, item: `https://rampseeker.com/texas/lakes/${lake.id}` }] }) }} />
      ...
      <p className="text-gray-500 mb-6">{lakeRamps.length} boat ramp{lakeRamps.length !== 1 ? "s" : ""} &middot; {lake.counties.join(", ")} Count{lake.counties.length > 1 ? "ies" : "y"}, Texas</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-12">
        {lakeRamps.map((r) => (
          <Link key={r.id} href={`/ramps/${r.id}`} className="group block bg-white border border-gray-200 rounded-xl p-5 border-l-4 border-l-sunset shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all">
            ...
            <p className="text-gray-500 text-sm">{r.city || "Texas"}</p>
            <span className="text-sm font-semibold text-sunset mt-2 inline-block">View Details &rarr;</span>
          </Link>
        ))}
        {lakeRamps.length === 0 && <p className="text-gray-400 col-span-3">No ramps found yet. <a href="mailto:hello@rampseeker.com" className="text-water">Know one?</a></p>}
      </div>
      ...
      <h2 className="font-[Cabin] text-xl font-bold text-charcoal mb-4">Other Texas Lakes</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {otherLakes.map((l) => (
          <Link key={l.id} href={`/texas/lakes/${l.id}`} className="group bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all border-l-4 border-l-water">
