...
export default function MarylandPage() {
  ...
  const cityMap = useMemo(() => {
    ...
    for (const r of mdRamps) { const c = r.city?.trim(); if (c && c.length > 1) m[c] = (m[c] || 0) + 1; }
  ...
  return (
    <div>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        mainEntity: [
          { "@type": "Question", name: "How many boat ramps are in Maryland?", acceptedAnswer: { "@type": "Answer", text: `There are ${mdRamps.length} boat ramps in Maryland. RampSeeker has mapped every public boat launch across the state.` } },
          { "@type": "Question", name: "Are boat ramps in Maryland free?", acceptedAnswer: { "@type": "Answer", text: "Many boat ramps in Maryland are free, especially those managed by state parks or the Army Corps of Engineers. Some may charge a launch fee." } },
          { "@type": "Question", name: "How do I find boat ramps near me in Maryland?", acceptedAnswer: { "@type": "Answer", text: `Use RampSeeker to browse all ${mdRamps.length} boat ramps in Maryland by city. Each listing includes a map and directions.` } },
        ],
      }) }} />
      <section className="py-16 md:py-24 text-center px-4 bg-cream" style={{ backgroundImage: "radial-gradient(circle at 20% 80%, rgba(30,96,145,0.06) 0%, transparent 50%)" }}>
        ...
        <p className="text-gray-500 mt-4 max-w-lg mx-auto">{mdRamps.length}+ boat ramps across {marylandLakes.length} lakes and waterways. Chesapeake Bay — largest estuary in the US, legendary rockfish, blue crab capital.</p>
      </section>

      {/* State intro */}
      <section className="max-w-4xl mx-auto px-4 pt-10 pb-2">
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm mb-6">
          <h2 className="font-[Cabin] text-xl font-bold text-charcoal mb-3">Boating in Maryland</h2>
          <p className="text-gray-600 leading-relaxed text-sm">Maryland offers {mdRamps.length.toLocaleString()}+ public boat ramps across its waterways. From the Chesapeake Bay, Deep Creek Lake, and the Potomac River, the state provides excellent access for boaters, anglers, and kayakers. Popular catches include striped bass, blue crabs, and catfish. <Link href="/blog/how-to-launch-a-boat-safely" className="text-water hover:underline">Learn how to launch safely</Link>.</p>
        </div>
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 mb-6">
          <h3 className="font-[Cabin] font-bold text-water mb-3">Tips for Boating in Maryland</h3>
          <ul className="space-y-2 text-sm text-gray-700">
            <li className="flex items-start gap-2"><span className="text-water mt-0.5">&#10003;</span> Maryland requires all motorized boats to be registered before launching at any public ramp.</li>
            <li className="flex items-start gap-2"><span className="text-water mt-0.5">&#10003;</span> A fishing license is required for anyone 16+ fishing from a boat in Maryland.</li>
            <li className="flex items-start gap-2"><span className="text-water mt-0.5">&#10003;</span> Life jackets are required for all children under 13 on any watercraft in Maryland.</li>
      ...
      <section className="max-w-4xl mx-auto px-4 py-10">
        ...
        <div className="space-y-2">
          {[
            { q: "How many boat ramps are in Maryland?", a: `RampSeeker lists ${mdRamps.length}+ boat ramps across Maryland, from the Chesapeake Bay to Deep Creek Lake.` },
            { q: "What are the best fishing spots in Maryland?", a: "Chesapeake Bay is legendary for rockfish (striped bass) and blue crabs. The Potomac River offers trophy bass fishing. Deep Creek Lake is the go-to for walleye and northern pike. Ocean City provides world-class offshore fishing." },
            { q: "Do I need a boating license in Maryland?", a: "Maryland requires a boating safety certificate for anyone born after July 1, 1972 who operates a motorized vessel. All motorized boats must be registered with the MD Department of Natural Resources." },
          
            { q: "Do I need a boating license in Maryland?", a: "Maryland requires a boating safety education certificate for boat operators. Check with the state's wildlife agency for specific age and horsepower requirements." },
            { q: "When is the best time to boat in Maryland?", a: "Peak boating season in Maryland runs from May through October, but many waterways are accessible year-round. Spring and fall offer less crowded conditions and excellent fishing." },
          ].map((f, i) => (
