...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: `${lake.name} Boat Ramps`, url: `https://rampseeker.com/nebraska/lakes/${lake.id}` },
    twitter: { card: "summary", title: `${lake.name} Boat Ramps | RampSeeker` },
    alternates: { canonical: `https://rampseeker.com/nebraska/lakes/${lake.id}` },
  };
}
...
export default async function NebraskaLakePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Home", item: "https://rampseeker.com" }, { "@type": "ListItem", position: 2, name: "Nebraska", item: "https://rampseeker.com/nebraska" }, { "@type": "ListItem", position: 3, name: lake.name, item: `https://rampseeker.com/nebraska/lakes/${lake.id}` }] }) }} />
