...
export default async function RampPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const gl = ramp.grandLakeData;
  const lake = getLakeForRamp(ramp.latitude, ramp.longitude) || (ramp.state === "TX" ? getTexasLakeForRamp(ramp.latitude, ramp.longitude) : undefined) || (ramp.state === "MO" ? getMissouriLakeForRamp(ramp.latitude, ramp.longitude) : undefined) || (ramp.state === "AR" ? getArkansasLakeForRamp(ramp.latitude, ramp.longitude) : undefined);
