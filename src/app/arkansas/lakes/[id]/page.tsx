...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: `${lake.name} Boat Ramps`, url: `https://rampseeker.com/arkansas/lakes/${lake.id}` },
    alternates: { canonical: `https://rampseeker.com/arkansas/lakes/${lake.id}` },
  };
}
...
export default async function ArkansasLakePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Home", item: "https://rampseeker.com" }, { "@type": "ListItem", position: 2, name: "Arkansas", item: "https://rampseeker.com/arkansas" }, { "@type": "ListItem", position: 3, name: lake.name, item: `https://rampseeker.com/arkansas/lakes/${lake.id}` }] }) }} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-12">
        {lakeRamps.map((r) => (
          <Link key={r.id} href={`/ramps/${r.id}`} className="group block bg-white border border-gray-200 rounded-xl p-5 border-l-4 border-l-sunset shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all">
            ...
            <p className="text-gray-500 text-sm">{r.city || "Arkansas"}</p>
