...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: `${lake.name} Boat Ramps`, url: `https://www.rampseeker.com/florida/lakes/${lake.id}` },
    twitter: { card: "summary", title: `${lake.name} Boat Ramps | RampSeeker` },
    alternates: { canonical: `https://www.rampseeker.com/florida/lakes/${lake.id}` },
  };
}
...
export default async function FloridaLakePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Home", item: "https://www.rampseeker.com" }, { "@type": "ListItem", position: 2, name: "Florida", item: "https://www.rampseeker.com/florida" }, { "@type": "ListItem", position: 3, name: lake.name, item: `https://www.rampseeker.com/florida/lakes/${lake.id}` }] }) }} />
