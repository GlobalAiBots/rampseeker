...
export default function FloridaPage() {
  ...
  const cityMap = useMemo(() => {
    ...
    for (const r of flRamps) { const c = r.city?.trim(); if (c && c.length > 1) m[c] = (m[c] || 0) + 1; }
  ...
  return (
    <div>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        mainEntity: [
          { "@type": "Question", name: "How many boat ramps are in Florida?", acceptedAnswer: { "@type": "Answer", text: `There are ${flRamps.length} boat ramps in Florida. RampSeeker has mapped every public boat launch across the state.` } },
          { "@type": "Question", name: "Are boat ramps in Florida free?", acceptedAnswer: { "@type": "Answer", text: "Many boat ramps in Florida are free, especially those managed by state parks or the Army Corps of Engineers. Some may charge a launch fee." } },
          { "@type": "Question", name: "How do I find boat ramps near me in Florida?", acceptedAnswer: { "@type": "Answer", text: `Use RampSeeker to browse all ${flRamps.length} boat ramps in Florida by city. Each listing includes a map and directions.` } },
      ...
      <section className="py-16 md:py-24 text-center px-4 bg-cream" style={{ backgroundImage: "radial-gradient(circle at 20% 80%, rgba(30,96,145,0.06) 0%, transparent 50%)" }}>
        ...
        <p className="text-gray-500 mt-4 max-w-lg mx-auto">{flRamps.length}+ boat ramps across {floridaLakes.length} lakes and coastal areas. Freshwater and saltwater launches.</p>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-4 pt-10 pb-2">
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm mb-6">
          <h2 className="font-[Cabin] text-xl font-bold text-charcoal mb-3">Boating in Florida</h2>
          <p className="text-gray-600 leading-relaxed text-sm">Florida offers {flRamps.length.toLocaleString()}+ public boat ramps across its waterways. From Lake Okeechobee, the Keys, and Tampa Bay, the state provides excellent access for boaters, anglers, and kayakers. Popular catches include bass, snook, redfish, and tarpon. <Link href="/blog/how-to-launch-a-boat-safely" className="text-water hover:underline">Learn how to launch safely</Link>.</p>
        </div>
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 mb-6">
          <h3 className="font-[Cabin] font-bold text-water mb-3">Tips for Boating in Florida</h3>
          <ul className="space-y-2 text-sm text-gray-700">
            <li className="flex items-start gap-2"><span className="text-water mt-0.5">&#10003;</span> Florida requires all motorized boats to be registered before launching at any public ramp.</li>
            <li className="flex items-start gap-2"><span className="text-water mt-0.5">&#10003;</span> A fishing license is required for anyone 16+ fishing from a boat in Florida.</li>
            <li className="flex items-start gap-2"><span className="text-water mt-0.5">&#10003;</span> Life jackets are required for all children under 13 on any watercraft in Florida.</li>
      ...
      <section className="max-w-6xl mx-auto px-4 pt-8 pb-8">
        <h2 className="font-[Cabin] text-2xl font-bold text-charcoal mb-4">Florida Lakes &amp; Coastal Areas</h2>
