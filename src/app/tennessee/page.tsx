...
export default function TennesseePage() {
  ...
  const cityMap = ((precomputedCities as unknown) as Record<string, [string, number][]>)["tennessee"] || [];
  ...
  return (
    <div>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        mainEntity: [
          { "@type": "Question", name: "How many boat ramps are in Tennessee?", acceptedAnswer: { "@type": "Answer", text: `There are ${tnRamps.length} boat ramps in Tennessee. RampSeeker has mapped every public boat launch across the state.` } },
          { "@type": "Question", name: "Are boat ramps in Tennessee free?", acceptedAnswer: { "@type": "Answer", text: "Many boat ramps in Tennessee are free, especially those managed by state parks or the Army Corps of Engineers. Some may charge a launch fee." } },
          { "@type": "Question", name: "How do I find boat ramps near me in Tennessee?", acceptedAnswer: { "@type": "Answer", text: `Use RampSeeker to browse all ${tnRamps.length} boat ramps in Tennessee by city. Each listing includes a map and directions.` } },
      ...
      <section className="max-w-4xl mx-auto px-4 pt-8">
        <p className="text-gray-600 leading-relaxed text-sm md:text-base">Tennessee offers {tnRamps.length.toLocaleString()}+ public boat ramps across its waterways. From Norris Lake, Cherokee Lake, and the Tennessee River, the state provides excellent access for boaters, anglers, and kayakers. Popular catches include bass, crappie, and catfish. <Link href="/blog/how-to-launch-a-boat-safely" className="text-water hover:underline">Learn how to launch safely</Link>.</p>
      </section>
      ...
      <div className="max-w-6xl mx-auto px-4"><FeaturedArticle listingSlug={"state-tennessee"} />
      <CletusAd /></div>
    </div>
  );
}
