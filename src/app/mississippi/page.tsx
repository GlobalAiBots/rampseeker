...
import { mississippiLakes, getMississippiLakeForRamp } from "@/data/mississippi-lakes";
...
export default function MississippiPage() {
  const stRamps = useMemo(() => unified.filter((r) => r.state === "MS"), []);

  const featuredLake = useMemo(() => {
    let best = mississippiLakes.find((l) => l.id === "ross-barnett-reservoir") || mississippiLakes[0]; let bestCount = 0;
    for (const l of mississippiLakes) {
      const count = stRamps.filter((r) => getMississippiLakeForRamp(r.latitude, r.longitude)?.id === l.id && !isGenericName(r.name)).length;
      if (count > bestCount) { best = l; bestCount = count; }
    }
    return best;
  }, [stRamps]);
  const featuredRamps = useMemo(() => featuredLake ? stRamps.filter((r) => getMississippiLakeForRamp(r.latitude, r.longitude)?.id === featuredLake.id && !isGenericName(r.name)) : [], [stRamps, featuredLake]);

  const lakeCounts = useMemo(() => {
    const map: Record<string, number> = {};
    for (const l of mississippiLakes) map[l.id] = stRamps.filter((r) => getMississippiLakeForRamp(r.latitude, r.longitude)?.id === l.id).length;
    return map;
  }, [stRamps]);

  const cityMap = ((precomputedCities as unknown) as Record<string, [string, number][]>)["mississippi"] || [];
  ...
  return (
    <div>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        mainEntity: [
          { "@type": "Question", name: "How many boat ramps are in Mississippi?", acceptedAnswer: { "@type": "Answer", text: `There are ${stRamps.length} boat ramps in Mississippi. RampSeeker has mapped every public boat launch across the state.` } },
          { "@type": "Question", name: "Are boat ramps in Mississippi free?", acceptedAnswer: { "@type": "Answer", text: "Many boat ramps in Mississippi are free, especially those managed by state parks or the Army Corps of Engineers. Some may charge a launch fee." } },
          { "@type": "Question", name: "How do I find boat ramps near me in Mississippi?", acceptedAnswer: { "@type": "Answer", text: `Use RampSeeker to browse all ${stRamps.length} boat ramps in Mississippi by city. Each listing includes a map and directions.` } },
      ...
      <section className="py-16 md:py-24 text-center px-4 bg-cream" style={{ backgroundImage: "radial-gradient(circle at 20% 80%, rgba(30,96,145,0.06) 0%, transparent 50%)" }}>
        <p className="text-forest text-sm font-bold tracking-wider uppercase mb-3 font-[Cabin]">Mississippi Boat Ramp Directory</p>
        <h1 className="font-[Cabin] text-4xl md:text-5xl font-bold text-charcoal leading-tight max-w-3xl mx-auto">Every Boat Ramp in Mississippi</h1>
        <p className="text-gray-500 mt-4 max-w-lg mx-auto">{stRamps.length}+ boat ramps across {mississippiLakes.length} lakes and waterways. Delta bass fishing, Gulf coast, reservoir crappie paradise.</p>
      </section>
      ...
      <section className="max-w-4xl mx-auto px-4 pt-8">
        <p className="text-gray-600 leading-relaxed text-sm md:text-base">Mississippi offers {stRamps.length.toLocaleString()}+ public boat ramps across its waterways. From Ross Barnett Reservoir, Grenada Lake, and the Gulf Coast, the state provides excellent access for boaters, anglers, and kayakers. Popular catches include bass, crappie, and redfish. <Link href="/blog/how-to-launch-a-boat-safely" className="text-water hover:underline">Learn how to launch safely</Link>.</p>
      </section>
      ...
      {featuredRamps.length > 0 && featuredLake && (
        <section className="max-w-6xl mx-auto px-4 pt-12 pb-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href={`/mississippi/lakes/${featuredLake.id}`} className="text-sm font-semibold text-sunset hover:text-sunset-dark transition hidden sm:block">View all &rarr;</Link>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {featuredRamps.slice(0, 6).map((r) => (
              <Link key={r.id} href={`/ramps/${r.id}`} className="group block bg-white border border-gray-200 rounded-xl p-5 border-l-4 border-l-sunset shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all">
                ...
                <p className="text-gray-500 text-sm mt-1">{r.city || "Mississippi"}{r.fee ? ` \u00b7 ${r.fee === "free" ? "Free" : r.fee}` : ""}</p>
      ...
      <section className="max-w-6xl mx-auto px-4 pt-8 pb-8">
        <h2 className="font-[Cabin] text-2xl font-bold text-charcoal mb-4">Mississippi Waters</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {mississippiLakes.sort((a, b) => (lakeCounts[b.id] || 0) - (lakeCounts[a.id] || 0)).map((l) => (
            <Link key={l.id} href={`/mississippi/lakes/${l.id}`} className="group bg-white border border-gray-200 rounded-xl p-4 hover:shadow-md hover:-translate-y-0.5 transition-all border-l-4 border-l-water">
      ...
      <div id="ramp-list"><RampList ramps={filteredRamps} stateName="Mississippi" /></div>

      {/* 7. TIPS */}
      <section className="max-w-4xl mx-auto px-4 pt-10 pb-2">
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 mb-6">
          <h3 className="font-[Cabin] font-bold text-water mb-3">Tips for Boating in Mississippi</h3>
          <ul className="space-y-2 text-sm text-gray-700">
            <li className="flex items-start gap-2"><span className="text-water mt-0.5">&#10003;</span> Mississippi requires all motorized boats to be registered before launching at any public ramp.</li>
            <li className="flex items-start gap-2"><span className="text-water mt-0.5">&#10003;</span> A fishing license is required for anyone 16+ fishing from a boat in Mississippi.</li>
            <li className="flex items-start gap-2"><span className="text-water mt-0.5">&#10003;</span> Life jackets are required for all children under 13 on any watercraft in Mississippi.</li>
      ...
      <section className="max-w-4xl mx-auto px-4 py-10">
        <h2 className="font-[Cabin] text-2xl font-bold text-charcoal mb-4">Mississippi Boating FAQ</h2>
        <div className="space-y-2">
          {[
            { q: "How many boat ramps are in Mississippi?", a: `RampSeeker lists ${stRamps.length}+ boat ramps across Mississippi.` },
      ...
      <div className="max-w-6xl mx-auto px-4"><FeaturedArticle listingSlug={"state-mississippi"} />
      <CletusAd /></div>
    </div>
  );
}
