...
import data from "./nebraska-lakes.json";

export const nebraskaLakes: Lake[] = data.map((l) => ({
...
export function getNebraskaLakeById(id: string): Lake | undefined {
  return nebraskaLakes.find((l) => l.id === id);
}

export function getNebraskaLakeForRamp(lat: number, lng: number): Lake | undefined {
